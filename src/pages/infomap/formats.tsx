...
import { CodeBlock, TabbedCodeBlock } from "../../shared/components/CodeBlock";
...
function FormatCard({ format }) {
  return (
    <DocsCard as="article" id={format.id} minW={0} maxW="100%">
      ...
      {format.examples ? (
        <TabbedCodeBlock
          ariaLabel={`${format.title} examples`}
          files={format.examples.map((example) => ({
            code: example.code,
            label: example.label,
          ...
          maxLines={15}
          renderFooter={(file) => {
            const example = format.examples.find(
              (item) => item.label === file.value,
            );
            if (!example) return null;

            return (
              <Text color="fg.muted" fontSize="sm" lineHeight={1.6} mt={2}>
                {example.description}
              </Text>
            );
          }}
